# Листинг 13.8, модуль data_manage.py
...
def initialize_note_table(db_file):
    with connect_to_db(db_file) as conn:
        sql = """ CREATE TABLE IF NOT EXISTS notes(id integer PRIMARY KEY,
         note_title text NOT NULL, 
        note_content text NOT NULL, date_written date NOT NULL );"""
...
def update_tasks_table(db_file, new_note):
    with connect_to_db(db_file) as conn:
        cursor = conn.cursor()
        sql_update_query = """UPDATE  notes SET note_title=?, note_content=?,
         date_written=? where id= ? """
